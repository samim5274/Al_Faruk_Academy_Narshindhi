    ...
    <div class="exam">
        ...
        <div class="table-wrap">
            <table>
                <thead>
                    <tr>
                        <th class="col-student">Student Name/Subject</th>
                ...
                @foreach($students as $std)
                    ...
                    <tr>
                        ...
                        @foreach($subjects as $sub)
                            @php
                                $mark = $lookup[$std->id][$sub->id] ?? null;
                                $rowTotal += is_numeric($mark) ? (float)$mark : 0;
                            @endphp
                            <td class="num">{{ is_null($mark) ? '-' : $mark }}</td>
                        @endforeach
                    ...
                    <tr>
                        ...
                        @foreach($subjects as $sub)
                            @php
                                ...
                                foreach($students as $std){
                                    $mark = $lookup[$std->id][$sub->id] ?? null;
                                    $colTotal += is_numeric($mark) ? (float)$mark : 0;
                                }

                                $grandTotal += $colTotal;
                            @endphp

                            <th class="num" style="background:#0f766e;color:#fff;">
                                {{ $colTotal }}
                            </th>
                        @endforeach
